...
public static class ContextHelper
{
    ...
    {
        if (pawn?.Map == null || pawn.Position == IntVec3.Invalid) return null;
        var room = pawn.GetRoom();
        return room is { PsychologicallyOutdoors: false } ? "Indoors".Translate() : "Outdoors".Translate();
    }
    public static Dictionary<Thought, float> GetThoughts(Pawn pawn)
    {
        var thoughts = new List<Thought>();
        pawn?.needs?.mood?.thoughts?.GetAllMoodThoughts(thoughts);
        return thoughts.GroupBy(t => t.def.defName).ToDictionary(g => g.First(), g => g.Sum(t => t.MoodOffset()));
    }
    public static string GetDecoratedName(Pawn pawn)
    {
        if (!pawn.RaceProps.Humanlike) return $"{pawn.LabelShort}(Age:{pawn.ageTracker.AgeBiologicalYears}, Race:{pawn.def.LabelCap})";
        var race = ModsConfig.BiotechActive && pawn.genes?.Xenotype != null ? pawn.genes.Xenotype.LabelCap : pawn.def.LabelCap;
        return $"{pawn.LabelShort}(Age:{pawn.ageTracker.AgeBiologicalYears}, Sex:{pawn.gender.GetLabel()}, Role:{pawn.GetRole(true)}, Race:{race})";
    }
    /*
    public static Building FindWallInFrontAndBack(Pawn pawn, int range)
    {
        ...
        for (int i = 1; i <= range; i++)
        {
            ...
            if (forwardCell.InBounds(pawn.Map))
            {
                ...
                if (edifice != null && IsWall(edifice)) return edifice;
            }
    ...
    */
    ...
    {
        if (pawn != null) text = text.Formatted(pawn.Named("PAWN")).AdjustedFor(pawn).Resolve();
    ...
    public static string FormatBackstory(string label, BackstoryDef backstory, Pawn pawn, InfoLevel infoLevel)
    {
        ...
        if (infoLevel == InfoLevel.Full) result += $":{Sanitize(backstory.description, pawn)}";
        return result;
    }
    ...
    public static List<string> CollectNearbyItems(Pawn pawn, int maxItems)
    {
        ...
        foreach (var cell in nearbyCells.InRandomOrder())
        {
            ...
            if (thingsHere == null || thingsHere.Count == 0) continue;
            // Skip cells with pawns/animals
            if (thingsHere.Any(t => t?.def != null && t.def.category != ThingCategory.Building && t.def.category != ThingCategory.Plant && t.def.category != ThingCategory.Item && !t.def.IsFilth)) continue;
            ...
            foreach (var thing in thingsHere)
            {
                ...
                var isValid = thing.def.category == ThingCategory.Building || thing.def.category == ThingCategory.Plant || thing.def.category == ThingCategory.Item || thing.def.IsFilth;
                if (!isValid) continue;
                if (thing.def.category == ThingCategory.Building && IsWall(thing))  continue;
                if (!candidatesByCategory.ContainsKey(thing.def.category)) candidatesByCategory[thing.def.category] = thing;
            }
            if (candidatesByCategory.Count == 0) continue;
            var picked = candidatesByCategory.Values.ToList().RandomElement();
            if (seenThings.Contains(picked)) continue;
            seenThings.Add(picked);
            if (picked is Building_Storage storage)
            {
                var stored = storage.AllSlotCells().SelectMany(c => c.GetThingList(pawn.Map)).Distinct().ToList();
                if (stored.Count > 0)
                {
            ...
            else items.Add(picked.LabelCap);
        }
        return items;
    }
}