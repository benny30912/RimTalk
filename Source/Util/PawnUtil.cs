...
public static class PawnUtil
{
    ...
    public static bool IsInDanger(this Pawn pawn, bool includeMentalState = false)
    {
        ...
        foreach (var h in pawn.health.hediffSet.hediffs) if (h.Visible && (h.CurStage?.lifeThreatening == true || h.def.lethalSeverity > 0 && h.Severity > h.def.lethalSeverity * 0.8f)) return true;
        return false;
    }
    ...
    public static string GetRole(this Pawn pawn, bool includeFaction = false)
    {
        ...
        if (pawn.IsEnemy())
            if (pawn.GetMapRole() == MapRole.Invading) return includeFaction && pawn.Faction != null ? $"Enemy Group({pawn.Faction.Name})" : "Enemy";
            else return "Enemy Defender";
    ...
    public static (string, bool) GetPawnStatusFull(this Pawn pawn, List<Pawn> nearbyPawns)
    {
        ...
        if (pawn.IsPlayer()) return ("Voice from beyond", false);
        ...
        if (pawn.IsFreeColonist && pawn.GetMapRole() == MapRole.Invading) lines.Add("Away from colony, attacking to enemy");
        else if (pawn.IsEnemy())
        {
            if (pawn.GetMapRole() == MapRole.Invading)
                if (pawn.GetLord()?.LordJob is LordJob_StageThenAttack || pawn.GetLord()?.LordJob is LordJob_Siege) lines.Add("waiting to invade user colony");
                else lines.Add("invading user colony");
            else lines.Add("Fighting to protect your home");
            return (string.Join("\n", lines), isInDanger);
        }
        ...
        if (nearestHostile != null)
        {
            ...
            if (distance <= 10f) lines.Add("Threat: Engaging in battle!");
            else if (distance <= 20f) lines.Add("Threat: Hostiles are dangerously close!");
            else lines.Add("Alert: hostiles in the area");
            isInDanger = true;
        }
        if (!isInDanger) lines.Add("Act based on role and context"); // NOTE: literal string instead of Constant.Prompt
        return (string.Join("\n", lines), isInDanger);
    ...
    public static Pawn GetHostilePawnNearBy(this Pawn pawn)
    {
        ...
        if (pawn.IsPrisoner || pawn.IsSlave || pawn.IsFreeColonist || pawn.IsVisitor() || pawn.IsQuestLodger()) referenceFaction = Faction.OfPlayer; // Prisoners, colonists, use player faction
        else referenceFaction = pawn.Faction; // enemies, wildmans, use own faction
        ...
        foreach (var target in hostileTargets.Where(target => GenHostility.IsActiveThreatTo(target, referenceFaction)))
        {
            ...
            if (threatPawn.IsPrisoner && threatPawn.HostFaction == Faction.OfPlayer) continue;
            if (threatPawn.IsSlave && threatPawn.HostFaction == Faction.OfPlayer) continue;
            // b. filter normal prisoner
            if (pawn.IsPrisoner && threatPawn.IsPrisoner) continue;
            Lord lord = threatPawn.GetLord();
            // === 1. EXCLUDE TACTICALLY RETREATING PAWNS ===
            if (lord != null && lord.CurLordToil is LordToil_ExitMapFighting or LordToil_ExitMap) continue;
            // === 2. EXCLUDE ROAMING MECH CLUSTER PAWNS ===
            if (threatPawn.RaceProps.IsMechanoid && lord is { CurLordToil: LordToil_DefendPoint }) continue;
    ...
    {
        if (pawn == null) return null;
        if (pawn.InMentalState) return pawn.MentalState?.InspectLine;
        if (pawn.CurJobDef is null) return null;
        var target = pawn.IsAttacking() ? pawn.TargetCurrentlyAimingAt.Thing?.LabelShortCap : null;
        if (target != null) return $"Attacking {target}";
    ...
    private static void AddJobTargetsToRelevantPawns(Job job, List<Pawn> relevantPawns)
    {
        ...
        foreach (TargetIndex ind in Enum.GetValues(typeof(TargetIndex)))
        {
            try
            {
                if (job.GetTarget(ind) != (LocalTargetInfo)(Thing)null) targetIndices.Add(ind);
            }
            catch
            {
    ...
    {
        if (pawn?.Map == null || pawn.IsPrisonerOfColony) return MapRole.None;
        Map map = pawn.Map;
        Faction mapFaction = map.ParentFaction;
        if (mapFaction == pawn.Faction || (map.IsPlayerHome && pawn.Faction == Faction.OfPlayer)) return MapRole.Defending; // player colonist
        if (pawn.Faction.HostileTo(mapFaction)) return MapRole.Invading;
