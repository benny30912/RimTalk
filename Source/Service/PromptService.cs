...
public static class PromptService
{
    ...
    public static string BuildContext(List<Pawn> pawns)
    {
        ...
        {
            var pawn = pawns[i];
            var pawnContext = pawn.IsPlayer() ? $"{pawn.LabelShort}\nRole: {pawn.GetRole()}" : CreatePawnContext(pawn, InfoLevel.Normal);
            Cache.Get(pawn).Context = pawnContext;
            context.AppendLine().AppendLine($"[Person {i + 1} START]").AppendLine(pawnContext).AppendLine($"[Person {i + 1} END]");
        }
    ...
    public static string CreatePawnBackstory(Pawn pawn, InfoLevel infoLevel = InfoLevel.Normal)
    {
        ...
        if (ModsConfig.BiotechActive && pawn.genes?.Xenotype != null) sb.AppendLine($"Race: {pawn.genes.Xenotype.LabelCap}");
        // Notable genes (Normal/Full only, not for enemies/visitors)
        if (infoLevel != InfoLevel.Short && !pawn.IsVisitor() && !pawn.IsEnemy() && ModsConfig.BiotechActive && pawn.genes?.GenesListForReading != null)
        {
            var notableGenes = pawn.genes.GenesListForReading.Where(g => g.def.biostatMet != 0 || g.def.biostatCpx != 0).Select(g => g.def.LabelCap);
            if (notableGenes.Any()) sb.AppendLine($"Notable Genes: {string.Join(", ", notableGenes)}");
        }
        // Ideology
        /*
        ...
        */
        ...
        if (pawn.IsEnemy() || pawn.IsVisitor()) return sb.ToString();
        // Backstory
        if (pawn.story?.Childhood != null) sb.AppendLine(ContextHelper.FormatBackstory("Childhood", pawn.story.Childhood, pawn, infoLevel));
        if (pawn.story?.Adulthood != null) sb.AppendLine(ContextHelper.FormatBackstory("Adulthood", pawn.story.Adulthood, pawn, infoLevel));
        // Traits
        ...
        foreach (var trait in pawn.story?.traits?.TraitsSorted ?? Enumerable.Empty<Trait>())
        {
            ...
            if (degreeData != null)
            {
                var traitText = infoLevel == InfoLevel.Full ? $"{degreeData.label}:{ContextHelper.Sanitize(degreeData.description, pawn)}" : degreeData.label;
        ...
        if (infoLevel != InfoLevel.Short)
        {
            ...
            if (skills?.Any() == true) sb.AppendLine($"Skills: {string.Join(", ", skills)}");
        }
        return sb.ToString();
    }
    private static string CreatePawnContext(Pawn pawn, InfoLevel infoLevel = InfoLevel.Normal)
    {
        ...
        if (personality != null) sb.AppendLine($"Personality: {personality}");
        //// INVADER STOP
        if (pawn.IsEnemy()) return sb.ToString();
        // Mood
        var m = pawn.needs?.mood;
        if (m?.MoodString != null)
        {
            string mood = pawn.Downed && !pawn.IsBaby() ? "Critical: Downed (in pain/distress)" : pawn.InMentalState ? $"Mood: {pawn.MentalState?.InspectLine} (in mental break)" : $"Mood: {m.MoodString} ({(int)(m.CurLevelPercentage * 100)}%)";
            sb.AppendLine(mood);
        }
        // Thoughts
        var thoughts = ContextHelper.GetThoughts(pawn).Keys.Select(t => ContextHelper.Sanitize(t.LabelCap));
        if (thoughts.Any()) sb.AppendLine($"Thoughts: {string.Join(", ", thoughts)}");
        if (pawn.IsSlave || pawn.IsPrisoner) sb.AppendLine(pawn.GetPrisonerSlaveStatus());
        // Visitor activity
        if (pawn.IsVisitor())
        {
        ...
        if (infoLevel != InfoLevel.Short)
        {
            ...
            if (pawn.equipment?.Primary != null) equipments.Add($"Weapon: {pawn.equipment.Primary.LabelCap}");
            var apparelLabels = pawn.apparel?.WornApparel?.Select(a => a.LabelCap);
            if (apparelLabels?.Any() == true) equipments.Add($"Apparel: {string.Join(", ", apparelLabels)}");
            if (equipments.Any()) sb.AppendLine($"Equipments: {string.Join(", ", equipments)}");
        }
        return sb.ToString();
    }
    ...
    public static void DecoratePrompt(TalkRequest talkRequest, List<Pawn> pawns, string status)
    {
        ...
        {
            if (pawns.Count == 1) sb.Append($"{shortName} short monologue");
            else if (mainPawn.IsInCombat() || mainPawn.GetMapRole() == MapRole.Invading)
            {
                if (talkRequest.TalkType != TalkType.Urgent && !mainPawn.InMentalState) talkRequest.Prompt = null;
            ...
            else  sb.Append($"{shortName} starts conversation, taking turns");
            // Modifiers
        ...
        */
        ...
        if (AIService.IsFirstInstruction()) sb.Append($"\nin {Constant.Lang}");
    ...
    public static List<(Role role, string message)> BuildMemoryBlockFromHistory(List<(Role role, string message)> history)
    {
        ...
        foreach (var (role, message) in history)
        {
            ...
            if (!string.IsNullOrWhiteSpace(ctx)) contexts.Add(ctx.Trim());
        }
